...
void Game::Update(float deltaTime)
{
	...
	if (m_pPlayer->GetDestinationTransform()->y < kHeight - m_pPlayer->GetDestinationTransform()->w - 48)
	{
		ground = true;
	}
	...
	else if (KeyDown(SDL_SCANCODE_D) && m_pPlayer->GetDestinationTransform()->x < kWidth - m_pPlayer->GetDestinationTransform()->w + 95)
	{
		m_pPlayer->GetDestinationTransform()->x += kPlayerSpeed * deltaTime;
	}
	if (KeyDown(SDL_SCANCODE_W) && m_pPlayer->GetDestinationTransform()->y * 3 > 0 && isfalling)
	{
		
		isfalling = false;
		m_pPlayer->GetDestinationTransform()->y -= kPlayerSpeed * 6 * deltaTime;
	}
	
	if (ground == true && isfalling == false)
	{
		if (/*KeyDown(SDL_SCANCODE_9) &&*/ m_pPlayer->GetDestinationTransform()->y < kHeight - m_pPlayer->GetDestinationTransform()->w - 48)
		{
			
			m_pPlayer->GetDestinationTransform()->y += 0.5 * (+ kPlayerSpeed  * deltaTime);
			cout << "Gravity is working ======" << endl;
		}
	}


	/*else if (KeyDown(SDL_SCANCODE_S) && m_pPlayer->GetDestinationTransform()->y < kHeight - m_pPlayer->GetDestinationTransform()->w - 48)
	{
